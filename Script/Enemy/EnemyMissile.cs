...
public class EnemyMissile : MonoBehaviour
{
    [Header("References")]
    [SerializeField]
    private EnemyFSM enemyFSM; // 부모 EnemyFSM 스크립트 참조
    private MemoryPool originPool; // 미사일을 관리하는 메모리 풀

    [Header("Values")]
    private float missileHP; // 미사일의 체력 (EnemySetting에서 missileHP 가져오기)
    private float missileMaxHP; // 미사일의 최대 체력
    private int missileDamage; // 미사일의 공격력 (EnemySetting에서 missileDamage 가져오기)
    ...
    // EnemyFSM에서 호출되는 Setup 함수
    // FSM에서 매개변수로 setting의 미사일 데미지, 체력을 전달받아 설정
    public void Setup(Vector3 targetPosition, int missileDamage, float missileHP, EnemyFSM enemyFSM)
    {
        ...
        this.missileHP = missileHP; // 미사일의 체력 설정
        this.missileMaxHP = missileHP; // 미사일의 최대 체력 설정
        ...
        childMissilePrefab.SetActive(true); // 자식 미사일 활성화
    }
    ...
    public void TakeDamage(float damage)
    {
        ...
        missileHP -= damage;
        missileHP = Mathf.Clamp(missileHP, 0, missileMaxHP);
